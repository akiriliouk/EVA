...
# The data looks more or less stationary.
```
...
gev.diag(gmonthly)
gev.diag(gyearly)
# I would go with the monthly maxima: the fit seems satisfactory 
# and confidence intervals will be narrower. 
```
...
qchisq(0.95, df = 1) 
1 - pchisq(Dmonthly, df =1)
1 - pchisq(Dyearly, df =1)
# cannot reject the Gumbel model
```
...
# hard to say based on above plots: slightly higher (70) may be better
mrl.plot(data, umin = 30, umax = 100)
# impossible to say something based on the mrl plot
...
2*4+ 2*M2$nllh
2*5+ 2*M3$nllh
# Including both time and the SOI gives the best model
```
...
gfit2$mle
gfit3$mle
# It seems that the inverse gamma distribution with shape parameter alpha
# is in the max domain of attraction of a GEV with shape 1/alpha

set.seed(1)
sample <- exp(1/runif(n = 100000, min = 0, max= 1))
summary(sample)
# too heavy-tailed to even fit a GEV
```